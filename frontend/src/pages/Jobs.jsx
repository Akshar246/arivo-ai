...
// Hybrid search (ChromaDB + Adzuna) via POST /jobs/search.
// Sponsorship verified against the Home Office Skilled Worker
// sponsor register.
//
// Self-contained: same backend contract as before. The only
// data a job carries is { title, company, location, salary,
// visa_sponsor, url } — so that's all this UI renders. Nothing
// is fabricated. Saved jobs + recent searches persist locally
// in the browser (no backend changes required).
// ─────────────────────────────────────────────────────────────

const SAVED_KEY = "arivo:savedJobs";
const RECENT_KEY = "arivo:recentSearches";

// Safe localStorage helpers (won't throw in private mode / SSR)
...
// Stable identity for a job (no id from the API, so derive one)
const jobKey = (j) => j.url || `${j.title}__${j.company}`;

// ── Inline icons (no dependency, themed via currentColor) ──────
const SearchIcon = () => (
  <svg
    width="18"
    height="18"
...
function Jobs() {
  ...
  const [sortBy, setSortBy] = useState("match"); // "match" | "sponsors"
  ...
  const runSearch = async (term) => {
    ...
    try {
      ...
      // Record the search (most recent first, deduped, capped at 6)
      const next = [
        q,
    ...
    <div className="aj">
          ...
          <div className="aj-context">
            {showSaved ? (
              ...
              <div className="aj-count">
                ...
                {sponsorCount > 0 && (
                  <span className="aj-count-sub">
                    {" "}
                    · {sponsorCount} sponsor visas
                  </span>
                )}
              </div>
            )}
            ...
            <div className="aj-card aj-card--skel" key={i}>
              <div className="aj-card-left">
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div
                    className="aj-skel"
                    style={{ width: 92, height: 18, borderRadius: 999 }}
                  />
                </div>
              </div>
              <div
      ...
      {!loading &&
        ...
        displayed.map((job) => {
          ...
          return (
            <article className="aj-card" key={jobKey(job)}>
              <div className="aj-card-left">
...
// Every color/spacing decision is a token on `.aj`.
// Pseudo-states live here because inline styles can't express them.
// ─────────────────────────────────────────────────────────────
const styles = `
.aj {
...
.aj button:focus-visible, .aj a:focus-visible {
...
/* Search bar — glows as one unit on focus (signature input) */
.aj-searchbar {
  display:flex; align-items:center; gap:8px; background:var(--surface);
  border:1px solid var(--border); border-radius:var(--r-field); padding:6px 6px 6px 16px;
...
.aj-clear {
  ...
  border:none; background:transparent; color:var(--text-3); cursor:pointer; border-radius:8px; transition:all .15s;
}
.aj-clear:hover { color:var(--text); background:rgba(255,255,255,.06); }
.aj-go {
  flex-shrink:0; padding:10px 22px; border:none; border-radius:10px;
  background:linear-gradient(135deg,var(--brand-1),var(--brand-2));
  color:#fff; font-size:13.5px; font-weight:600; cursor:pointer; transition:filter .18s, transform .12s;
}
...
.aj-chip {
  ...
  border:1px solid var(--border); color:var(--text-2); font-size:12.5px; transition:all .16s;
}
.aj-chip:hover { border-color:var(--border-hi); color:var(--text); background:rgba(102,126,234,.08); transform:translateY(-1px); }
 
/* Saved entry */
.aj-saved-entry {
  display:inline-flex; align-items:center; gap:9px; margin-top:1.75rem; padding:10px 16px;
...
.aj-count { font-size:13px; color:var(--text-2); }
.aj-count strong { color:var(--text); font-weight:600; }
.aj-count-sub { color:var(--text-3); }
...
.aj-toggle {
  ...
  background:transparent; border:1px solid var(--verify-1); color:var(--verify-1); font-size:12.5px; font-weight:600; transition:all .18s;
}
...
.aj-card {
  ...
  padding:18px 20px; margin-bottom:12px; transition:transform .18s, border-color .18s, background .18s;
}
.aj-card:not(.aj-card--skel):hover { transform:translateY(-2px); border-color:var(--border-hi); background:var(--surface-2); }
...
.aj-logo {
  ...
  border:1px solid rgba(102,126,234,.30); color:var(--brand-1); font-size:17px; font-weight:700;
}
...
.aj-save {
  ...
  border:1px solid var(--border); background:transparent; color:var(--text-3); border-radius:10px; cursor:pointer; transition:all .18s;
}
...
.aj-apply {
  display:inline-flex; align-items:center; gap:6px; padding:9px 16px; border-radius:10px; text-decoration:none;
  background:linear-gradient(135deg,var(--brand-1),var(--brand-2)); color:#fff; font-size:12.5px; font-weight:600; transition:filter .18s;
}
...
.aj-retry {
  margin-top:14px; padding:8px 18px; border:none; border-radius:9px;
  background:linear-gradient(135deg,var(--brand-1),var(--brand-2)); color:#fff; font-size:12.5px; font-weight:600; cursor:pointer; transition:filter .18s;
}
...
@media (max-width:560px) {
  ...
  .aj-card { flex-direction:column; align-items:stretch; }
